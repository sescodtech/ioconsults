...
export default function WhatsAppButton() {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-40 flex items-end gap-3">
      <div className="hidden sm:block max-w-[220px] bg-forest text-bone text-sm px-4 py-3 shadow-lg relative">
        <button
          type="button"
          onClick={() => setDismissed(true)}
          aria-label="Dismiss"
          className="absolute -top-2 -right-2 w-5 h-5 flex items-center justify-center bg-ink text-bone text-xs rounded-full"
        >
          ✕
        </button>
        <p>
          <span className="font-medium">Hi there!</span> Chat with us on
          WhatsApp — we respond swiftly.
        </p>
      </div>
      <a
